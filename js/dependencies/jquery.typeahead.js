  ...
  Typeahead.prototype = {
    extendOptions: function () {
      ...
      if (
        ...
        (!/^\d+$/.test(this.options.maxItem) || this.options.maxItem === 0)
      ) {
    ...
    unifySourceFormat: function () {
      ...
      for (var group in this.options.source) {
        if (!Object.hasOwn(this.options.source, group)) continue
    ...
    delegateEvents: function () {
      ...
      this.container.off(_namespace).on('click' + _namespace + ' touchstart' + _namespace, (e) => {
        e.stopPropagation()

        if (scope.options.dropdownFilter) {
          scope.container.find('.' + scope.options.selector.dropdown.replace(' ', '.')).hide()
        }
      })
      ...
      this.node.off(_namespace).on(events.join(' '), (e) => {
        switch (e.type) {
          case 'generateOnLoad':
          case 'focus':
    ...
    generateSource: function () {
      ...
      if (!this.helper.isEmpty(this.xhr)) {
        for (var i in this.xhr) {
          if (!Object.hasOwn(this.xhr, i)) continue
          this.xhr[i].abort()
        }
        this.xhr = {}
      }

      var group, dataInLocalstorage, isValidStorage
      ...
      for (group in this.options.source) {
        ...
        if (this.options.source[group].data && !this.options.source[group].url) {
          this.populateSource(
            (typeof this.options.source[group].data === 'function' && this.options.source[group].data()) ||
    ...
    generateRequestObject: function (group) {
      ...
      for (var _group in this.requests) {
        if (!Object.hasOwn(this.requests, _group)) continue
    ...
    handleRequests: function () {
      var requestsCount = Object.keys(this.requests).length
      ...
      for (var group in this.requests) {
        if (!Object.hasOwn(this.requests, group)) continue
        ...
        ;((group, xhrObject) => {
          ...
          if (xhrObject.request.data) {
            for (var i in xhrObject.request.data) {
              if (!Object.hasOwn(xhrObject.request.data, i)) continue
              if (~String(xhrObject.request.data[i]).indexOf('{{query}}')) {
                xhrObject = $.extend(true, {}, xhrObject)
                xhrObject.request.data[i] = xhrObject.request.data[i].replace('{{query}}', this.query)
                break
              }
            }
          }

          this.xhr[group] = $.ajax(xhrObject.request)
            .done((data, textStatus, jqXHR) => {
              var tmpData
              for (var i = 0; i < xhrObject.validForGroup.length; i++) {
                _request = this.requests[xhrObject.validForGroup[i]]
                ...
                this.populateSource(data, _request.extra.group, _request.extra.path)

                requestsCount -= 1
                if (requestsCount === 0) {
                  this.helper.executeCallback(this.options.callback.onReceiveRequest, [this.node, this.query])
                }
              }
            })
            .fail((jqXHR, textStatus, errorThrown) => {
              for (var i = 0; i < xhrObject.validForGroup.length; i++) {
                _request = this.requests[xhrObject.validForGroup[i]]
                _request.extra.callback.fail instanceof Function &&
                  _request.extra.callback.fail(jqXHR, textStatus, errorThrown)
              }
              _debug.log({
                node: this.node.selector,
            ...
            .complete((jqXHR, textStatus) => {
              for (var i = 0; i < xhrObject.validForGroup.length; i++) {
                _request = this.requests[xhrObject.validForGroup[i]]
                _request.extra.callback.complete instanceof Function &&
                  _request.extra.callback.complete(jqXHR, textStatus)
              }
            })
            .always((data, textStatus, jqXHR) => {
              for (var i = 0; i < xhrObject.validForGroup.length; i++) {
                _request = this.requests[xhrObject.validForGroup[i]]
                _request.extra.callback.always instanceof Function &&
                  _request.extra.callback.always(data, textStatus, jqXHR)
              }
            })
    ...
    populateSource: function (data, group, path) {
      ...
      if (extraData) {
        ...
        if (extraData instanceof Array) {
          ...
          _debug.log({
            ...
            message:
              'WARNING - this.options.source.' + group + '.data Must be an Array or a function that returns an Array.',
          })
      ...
      if (this.options.cache && !localStorage.getItem(this.node.selector + ':' + group)) {
        ...
        localStorage.setItem(this.node.selector + ':' + group, storage)
      }
    ...
    searchResult: function () {
      ...
      for (group in this.source) {
        if (!Object.hasOwn(this.source, group)) continue
        ...
        for (var k = 0; k < this.source[group].length; k++) {
          ...
          for (var i = 0; i < displayKeys.length; i++) {
            ...
            if (
              (this.options.callback.onResult && this.result.length >= this.options.maxItem) ||
      ...
      if (this.options.order) {
        ...
        this.result.sort(this.helper.sort(displayKeys, this.options.order === 'asc', (a) => a.toString().toUpperCase()))
      }

      this.helper.executeCallback(this.options.callback.onResult, [
        this.node,
        this.query,
        this.result,
        this.resultCount,
    ...
    buildLayout: function () {
        ...
        resultHtmlList = $('<ul/>', {
          ...
          html: function () {
            if (scope.options.emptyTemplate && scope.helper.isEmpty(scope.result)) {
              return $('<li/>', {
                html: $('<a/>', {
                  href: 'javascript:;',
                  html:
                    (typeof scope.options.emptyTemplate === 'function' && scope.options.emptyTemplate(scope.query)) ||
            ...
            for (var i in scope.result) {
              if (!Object.hasOwn(scope.result, i)) continue
                    ...
                    $(ulScope).append(
                      $('<li/>', {
                        ...
                        html: $('<a/>', {
                          href: 'javascript:;',
                          html:
                            (scope.options.group[1] &&
                              scope.options.group[1].replace(
                                /(\{\{group}})/gi,
                                item[scope.options.group[0]] || _group,
                              )) ||
                            _group,
                        }),
                ...
                for (var i = 0; i < _displayKeys.length; i++) {
                  ...
                  if (scope.options.highlight) {
                    if (_display[_displayKey]) {
                      _display[_displayKey] = scope.helper.highlight(
                        _display[_displayKey],
                        _query,
                ...
                _liHtml = $('<li/>', {
                  html: $('<a/>', {
                    href: () => {
                      if (_href) {
                        if (typeof _href === 'string') {
                          _href = _href.replace(/\{\{([a-z0-9_\-.]+)\|?(\w+)?}}/gi, (match, index, option) => {
                    ...
                    html: function () {
                      ...
                      if (_template) {
                        _aHtml = _template.replace(/\{\{([a-z0-9_\-.]+)\|?(\w+)?}}/gi, (match, index, option) => {
                          var value = scope.helper.namespace(index, item, 'get') || match
                          if (option && option === 'raw') {
                            return value
                          }
                        ...
                        _aHtml =
                          ...
                          '">' +
                          scope.helper.joinObject(_display, ' ') +
                          '</span>'
                      }
      ...
      if (this.options.callback.onLayoutBuiltBefore) {
        var tmpResultHtmlList = this.helper.executeCallback(this.options.callback.onLayoutBuiltBefore, [
          this.node,
          this.query,
          this.result,
          resultHtmlList,
      ...
      this.resultContainer.html(resultHtmlList)
      ...
      if (this.options.hint) {
        ...
        if (this.hint.container) {
          this.hint.container.val((_hint.length > 0 && this.rawQuery + _hint.substring(this.query.length)) || '').show()
        }
      }
    },
        ...
        html: function () {
          $(this).append(
            $('<button/>', {
              ...
              html:
                "<span class='" +
                scope.options.selector.filterValue +
                "'>" +
                defaultText +
                "</span> <span class='" +
                scope.options.selector.dropdownCarret +
                "'></span>",
              click: (e) => {
                e.stopPropagation()
                ...
                if (!filterContainer.is(':visible')) {
                  ...
                  $('html')
                    .off(_namespace + '.dropdownFilter')
                    .on(
                      'click' + _namespace + '.dropdownFilter' + ' touchstart' + _namespace + '.dropdownFilter',
                      function () {
              ...
              html: function () {
                ...
                if (~['string', 'boolean'].indexOf(typeof scope.options.dropdownFilter)) {
                  ...
                  for (var group in scope.options.source) {
                    if (!Object.hasOwn(scope.options.source, group)) continue
                    items.push({
                      key: 'group',
                      value: group,
                    })
                  ...
                  items.push({
                    ...
                    value: '*',
                    display:
                      (typeof scope.options.dropdownFilter === 'string' && scope.options.dropdownFilter) || 'All',
                  })
                }
    ...
    dynamicFilter: {
      validate: function (item) {
        ...
        for (var key in this.filters.dynamic) {
          if (!Object.hasOwn(this.filters.dynamic, key)) continue
          if (~key.indexOf('.')) {
    ...
    showLayout: function () {
      ...
      $('html')
        .off(_namespace)
        .on('click' + _namespace + ' touchstart' + _namespace, function () {
          scope.hideLayout()
          $(this).off(_namespace)
        })
    ...
    helper: {
      ...
      removeAccent: (string) => {
        ...
        string = string
          .toLowerCase()
          .replace(new RegExp('[' + _accent.from + ']', 'g'), (match) => _accent.to[_accent.from.indexOf(match)])
      ...
      replaceAt: (string, offset, length, replace) =>
        string.substring(0, offset) + replace + string.substring(offset + length),
      ...
      highlight: function (string, key, accents) {
        ...
        return this.replaceAt(string, offset, key.length, '<strong>' + string.substr(offset, key.length) + '</strong>')
      },

      joinObject: (object, join) => {
        var string = '',
          iteration = 0

        for (var i in object) {
          if (!Object.hasOwn(object, i)) continue
      ...
      namespace: (namespaceString, objectReference, method, objectValue) => {
